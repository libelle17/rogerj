			...
			gpointer      user_data)
{
	...
	if (item->state == SOUP_MESSAGE_CONNECTED ||
	    item->state == SOUP_MESSAGE_READY) {
    printf("connect_async_complete 1\n");
		async_run_queue (item->session);
    printf("connect_async_complete 2\n");
	} else {
		soup_session_kick_queue (item->session);
  }

	soup_message_queue_item_unref (item);
    printf("connect_async_complete 3\n");
}
...
get_connection (SoupMessageQueueItem *item, gboolean *should_cleanup)
{
  ...
  printf("get_connection 2\n");
	g_mutex_lock (&priv->conn_lock);
  printf("get_connection 21\n");
	host = get_host_for_message (session, item->msg);
  printf("get_connection 22\n");
	while (TRUE) {
  printf("get_connection 23\n");
		conn = get_connection_for_host (session, item, host,
						need_new_connection,
						ignore_connection_limits,
						&my_should_cleanup,
						&is_dedicated_connection);
  printf("get_connection 24, conn: %i, item->async: %i\n",(int)(conn),(int)(item->async));
	if (conn || item->async)
			break;

  printf("get_connection 25\n");
		if (my_should_cleanup) {
  printf("get_connection 26\n");
			g_mutex_unlock (&priv->conn_lock);
			soup_session_cleanup_connections (session, TRUE);
			g_mutex_lock (&priv->conn_lock);

			my_should_cleanup = FALSE;
  printf("get_connection 27\n");
			continue;
		}
  printf("Achtung!\n");
  p1=priv->conn_cond.p?priv->conn_cond.p:"";
  printf("p1: %5.5s\n",p1);
  p2=priv->conn_lock.p?priv->conn_lock.p:"";
  printf("vor p2, sizeof p2 %lu\n",sizeof p2);
  hexDump(p2,&p2,sizeof(p2));
  printf("get_connection 28, cond: %i, lock: %i\n",*priv->conn_cond.i,*priv->conn_lock.i);
		g_cond_wait (&priv->conn_cond, &priv->conn_lock);
  printf("get_connection 29\n");
	}
  printf("get_connection 3\n");
	g_mutex_unlock (&priv->conn_lock);
  printf("get_connection 4\n");
...
soup_session_process_queue_item (SoupSession          *session,
				 ...
				 gboolean              loop)
{
  printf("soup_session_process_queue_item 1\n");
	g_assert (item->session == session);
  printf("soup_session_process_queue_item 2\n");

	do {
  printf("soup_session_process_queue_item 3\n");
		if (item->paused)
			return;

  printf("soup_session_process_queue_item 4\n");
		switch (item->state) {
		case SOUP_MESSAGE_STARTING:
  printf("soup_session_process_queue_item 5\n");
			if (!get_connection (item, should_cleanup)) {
  printf("soup_session_process_queue_item 51\n");
				return;
      }
  printf("soup_session_process_queue_item 52\n");
			break;

		case SOUP_MESSAGE_CONNECTED:
  printf("soup_session_process_queue_item 6\n");
  ...
  printf("soup_session_process_queue_item 7\n");
			if (item->msg->status_code) {
  ...
  printf("soup_session_process_queue_item 8\n");
			item->state = SOUP_MESSAGE_RUNNING;
  printf("soup_session_process_queue_item 9\n");
  ...
  printf("soup_session_process_queue_item 12\n");
			if (item->async)
				return;

			g_warn_if_fail (item->new_api);
  printf("soup_session_process_queue_item 13\n");
			item->state = SOUP_MESSAGE_FINISHING;
			break;

		case SOUP_MESSAGE_CACHED:
  printf("soup_session_process_queue_item 14\n");
			/* Will be handled elsewhere */
			return;

		case SOUP_MESSAGE_RESTARTING:
  printf("soup_session_process_queue_item 15\n");
			item->state = SOUP_MESSAGE_STARTING;
			soup_message_restarted (item->msg);
			break;

		case SOUP_MESSAGE_FINISHING:
  printf("soup_session_process_queue_item 16\n");
			item->state = SOUP_MESSAGE_FINISHED;
			...
			soup_message_queue_item_ref (item);
  printf("soup_session_process_queue_item 17\n");
			soup_session_unqueue_item (session, item);
  printf("soup_session_process_queue_item 171\n");
			if (item->async && item->callback) {
  printf("soup_session_process_queue_item 172\n");
				item->callback (session, item->msg, item->callback_data);
  printf("soup_session_process_queue_item 173\n");
      }
  printf("soup_session_process_queue_item 174\n");
			soup_message_queue_item_unref (item);
  printf("soup_session_process_queue_item 175\n");
			return;

		default:
  printf("soup_session_process_queue_item 18\n");
  ...
  printf("soup_session_process_queue_item 19\n");
}
...
idle_run_queue (gpointer user_data)
{
    ...
    printf("idle_run_queue 1\n");
	async_run_queue (session);
    printf("idle_run_queue 2\n");
	g_object_unref (session);
    printf("idle_run_queue 3\n");
	return FALSE;
}
...
soup_session_real_send_message (SoupSession *session, SoupMessage *msg)
{
  ...
  printf("soup_session_real_send_message 1\n");
	item = soup_session_append_queue_item (session, msg, FALSE, FALSE,
					       NULL, NULL);
  printf("soup_session_real_send_message 2\n");
	soup_session_process_queue_item (session, item, NULL, TRUE);
  printf("soup_session_real_send_message 3\n");
	status = msg->status_code;
  printf("soup_session_real_send_message 4\n");
	soup_message_queue_item_unref (item);
  printf("soup_session_real_send_message 5\n");
	return status;
}
...
{
  /*GSchade*/SoupMessageHeadersIter iter; const char *nam, *val;
  printf("soup_session_send_message msg: method '%s', status_code: '%d', reason_phrase: '%s', request_body: '%s', request_body->length: '%ld'\n",msg->method, msg->status_code, msg->reason_phrase, msg->request_body->data, msg->request_body->length);
  g_print ("%s HTTP/1.%d\n", msg->method, soup_message_get_http_version (msg));
  soup_message_headers_iter_init (&iter, msg->request_headers);
  while (soup_message_headers_iter_next (&iter, &nam, &val))
    g_print ("%s: %s\n", nam, val);

  if (msg->request_body->length)
  g_print ("%s\n", msg->request_body->data);
	g_return_val_if_fail (SOUP_IS_SESSION (session), SOUP_STATUS_MALFORMED);
  printf("soup_session_send_message 2\n");
	g_return_val_if_fail (SOUP_IS_MESSAGE (msg), SOUP_STATUS_MALFORMED);
  printf("soup_session_send_message 3\n");
...
soup_session_real_kick_queue (SoupSession *session)
{
    ...
    printf("soup_session_real_kick_queue 1\n");
					source = soup_add_completion_reffed (context, idle_run_queue, wref, idle_run_queue_dnotify);
    printf("soup_session_real_kick_queue 2\n");
					g_source_unref (source);
				}
    printf("soup_session_real_kick_queue 3\n");
				g_hash_table_add (async_pending, context);
			}
    printf("soup_session_real_kick_queue 4\n");
			item->async_pending = TRUE;
		} else
			have_sync_items = TRUE;
    printf("soup_session_real_kick_queue 5\n");
	}
    printf("soup_session_real_kick_queue 6\n");
	g_hash_table_unref (async_pending);
    printf("soup_session_real_kick_queue 7\n");

	if (have_sync_items) {
    printf("soup_session_real_kick_queue 8\n");
		g_mutex_lock (&priv->conn_lock);
    printf("soup_session_real_kick_queue 9\n");
		g_cond_broadcast (&priv->conn_cond);
    printf("soup_session_real_kick_queue 10\n");
		g_mutex_unlock (&priv->conn_lock);
    printf("soup_session_real_kick_queue 11\n");
	}
    printf("soup_session_real_kick_queue 12\n");
}
			  ...
			  GError       **error)
{
	...
	if (g_task_had_error (task)) {
		...
		if (item->state != SOUP_MESSAGE_FINISHED) {
      printf("soup_session_send_finish 1\n");
			soup_session_process_queue_item (session, item, NULL, FALSE);
      printf("soup_session_send_finish 2\n");
    }
	}
		   ...
		   GError       **error)
{
    ...
    printf("soup_session_send 1\n");
		soup_session_process_queue_item (session, item, NULL, TRUE);
    printf("soup_session_send 2\n");
		if (item->state != SOUP_MESSAGE_RUNNING)
			break;
	...
	if (!stream) {
		...
		if (item->state != SOUP_MESSAGE_FINISHED) {
    printf("soup_session_send 3\n");
			soup_session_process_queue_item (session, item, NULL, TRUE);
    printf("soup_session_send 4\n");
    }
	}
