...
// verbergen: 0 = nichts, 1= '2>/dev/null' anhaengen + true zurueckliefern, 2='>/dev/null 2>&1' anhaengen + Ergebnis zurueckliefern
  ...
  if (ueberschr.empty()) { 
	...
	string meldbef=(obsudc?sudc+(obsudc==2&&!sudc.empty()?"-H ":""):"")+
		(obdirekt?hcmd:"env PATH='...' sh -c '"+ersetzAllezu(hcmd,"'","'\\''")+"'");
	meldbef=meldbef.substr(0,getcols()-7-aktues.length());
	string meld=aktues+": "+blau+meldbef+schwarz+" ...";
	if (ausgp&&obverb) *ausgp<<meldbef<<endl; else Log(meldbef,obverb>0?-1:0,oblog);
