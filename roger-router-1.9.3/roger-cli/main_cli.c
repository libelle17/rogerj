...
int main(int argc, char **argv)
{
  ...
  */
  ...
  printf("Hier roger_cli 11\n");

	/* Create and start g_main_loop */
	main_loop = g_main_loop_new(NULL, FALSE);
  printf("Hier roger_cli 12\n");
	...
	gboolean sff = g_settings_get_boolean(profile->settings, "fax-sff");
  printf("Achtung: %d %d %d %d %s %s %s %d\n",modem,ecm,controller,cip,src_no,header,ident);
	controller = g_settings_get_int(profile->settings, "phone-controller") + 1;
	src_no = g_settings_get_string(profile->settings, "phone-number");
  printf("Achtung: %d %s\n",controller,src_no);
	g_main_loop_run(main_loop);
  printf("Hier roger_cli 13\n");

	/* Shutdown routermanager */
	routermanager_shutdown();
  printf("Hier roger_cli 14\n");

	if (sendfax && tiff) {
		g_unlink(tiff);
	}
  printf("Hier roger_cli 15\n");

	if (sendfax && !success) {
		ret = -6;
	}
  printf("Hier roger_cli 16\n");
