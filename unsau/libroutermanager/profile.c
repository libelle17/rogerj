...
gboolean profile_detect_active(void)
{
  ...
  printf("profile_detect_active 2 %s\n",g_settings_get_string(profile->settings, "serial"));
		if (router_present(router_info) == TRUE && !strcmp(router_info->serial, g_settings_get_string(profile->settings, "serial"))) {
  printf("profile_detect_active 3\n");
  ...
  printf("profile_detect_active 2 %s!=%s\n",router_info->serial,g_settings_get_string(profile->settings, "serial"));
