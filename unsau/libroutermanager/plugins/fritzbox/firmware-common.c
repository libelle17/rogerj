...
// folgendes bis fritzbox_present GSchade 
...
static void
get_url (const char *url,SoupMessage *msg)
{
	const char *name;
	const char *header;
...
gboolean fbp(struct router_info *router_info,SoupMessage *msg)
{
  static gboolean synchronous=TRUE;
  char *url = g_strdup_printf("http://%s/jason_boxinfo.xml", router_info->host);
...
gboolean fritzbox_present(struct router_info *router_info)
{
  ...
  printf("fritzbox_present 2\n");

  printf("1 fritzbox_present %s\n",router_info->host);
	url = g_strdup_printf("http://%s/jason_boxinfo.xml", router_info->host);
  printf("Nr 2 fritzbox_present, url: '%s'\n",url);
	msg = soup_message_new(SOUP_METHOD_GET, url);
  // fbp(router_info,msg);
  /*
  printf("Nr 3 fritzbox_present, msg: method '%s', status_code: '%d', reason_phrase: '%s', request_body: '%s', length: '%ld',\n request_header: '%s', response_body: '%s', length: '%ld'\n",msg->method, msg->status_code, msg->reason_phrase, msg->request_body->data, msg->request_body->length, (char*)msg->response_body, (char*)msg->response_body->data, msg->response_body->length);
*/
...
//				SOUP_SESSION_ADD_FEATURE_BY_TYPE, SOUP_TYPE_COOKIE_JAR,
  ...
  soup_session_send_message(soup_session, msg);
      g_object_unref (session);
#endif
  printf("Nr 4 fritzbox_present\n");
	if (msg->status_code != 200) {
  printf("Nr 4_1 fritzbox_present\n");
		g_object_unref(msg);
  printf("Nr 4_2 fritzbox_present\n");
		g_free(url);
  printf("Nr 4_3 fritzbox_present\n");

		if (msg->status_code == 404) {
  printf("Nr 4_4 fritzbox_present\n");
			ret = fritzbox_present_04_00(router_info);
		} else {
  printf("Nr 4_5 fritzbox_present\n");
			g_warning("Could not read boxinfo file (Error: %d, %s)", msg->status_code, soup_status_get_phrase(msg->status_code));
  printf("Nr 4_6 fritzbox_present\n");
		}

  printf("Nr 4_7 fritzbox_present\n");
		return ret;
	}
  printf("Nr 5 fritzbox_present\n");
	data = msg->response_body->data;
	read = msg->response_body->length;

  printf("Nr 6 fritzbox_present\n");
	log_save_data("fritzbox-present.html", data, read);
	g_return_val_if_fail(data != NULL, FALSE);

  printf("Nr 7 fritzbox_present\n");
  ...
  printf("Nr 8 fritzbox_present\n");
	g_object_unref(msg);
	g_free(url);

  printf("Nr 9 fritzbox_present\n");
  ...
  printf("Nr 11 fritzbox_present\n");
	g_free(annex);
	g_free(serial);
	g_free(lang);
	g_free(version);
	g_free(name);
  printf("Nr 12 fritzbox_present\n");
