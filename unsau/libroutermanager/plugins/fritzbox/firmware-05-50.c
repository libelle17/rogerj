...
static gboolean fritzbox_check_login_blocked(const gchar *data, struct profile *profile)
{
  ...
  printf("1 fritzbox_check_login_blocked\n");
	g_debug("%s(): session_id %s", __FUNCTION__, profile->router_info->session_id);
  printf("2 fritzbox_check_login_blocked\n");
	result = !!strcmp(profile->router_info->session_id, "0000000000000000");
  printf("3 fritzbox_check_login_blocked\n");

	if (!result) {
  printf("4 fritzbox_check_login_blocked\n");
		const gchar *blocktime = xml_extract_tag(data, "BlockTime");
  printf("5 fritzbox_check_login_blocked\n");

		if (blocktime) {
  printf("6 fritzbox_check_login_blocked\n");
			...
			if (atoi(blocktime)) {
  printf("7 fritzbox_check_login_blocked\n");
				g_timer_destroy(profile->router_info->session_timer);
				profile->router_info->session_timer = NULL;
			}
  printf("8 fritzbox_check_login_blocked\n");
		}
  printf("10 fritzbox_check_login_blocked\n");
	}
  printf("11 fritzbox_check_login_blocked\n");
...
gboolean fritzbox_login_05_50(struct profile *profile)
{
  ...
  printf("3 fritzbox_login_05_50\n");
		if (!profile->router_info->session_timer) {
  printf("4 fritzbox_login_05_50\n");
			profile->router_info->session_timer = g_timer_new();
  printf("5 fritzbox_login_05_50\n");
			g_timer_start(profile->router_info->session_timer);
  printf("6 fritzbox_login_05_50\n");
		} else {
  printf("7 fritzbox_login_05_50\n");
			g_timer_reset(profile->router_info->session_timer);
		}
	}

  printf("8 fritzbox_login_05_50\n");
	url = g_strdup_printf("http://%s/login_sid.lua", router_get_host(profile));
  printf("9 fritzbox_login_05_50\n");
	msg = soup_message_new(SOUP_METHOD_GET, url);
  printf("10 fritzbox_login_05_50\n");
	g_free(url);
  printf("11 fritzbox_login_05_50\n");

  printf("12 fritzbox_login_05_50\n");
	soup_session_send_message(soup_session, msg);
  printf("13 fritzbox_login_05_50\n");
	if (msg->status_code != 200) {
  printf("14 fritzbox_login_05_50\n");
		g_debug("%s(): Received status code: %d", __FUNCTION__, msg->status_code);
  printf("15 fritzbox_login_05_50\n");
		g_object_unref(msg);
  printf("16 fritzbox_login_05_50\n");

		g_timer_destroy(profile->router_info->session_timer);
  printf("17 fritzbox_login_05_50\n");
		profile->router_info->session_timer = NULL;
  printf("18 fritzbox_login_05_50\n");
		return FALSE;
	}
  printf("19 fritzbox_login_05_50\n");
	data = msg->response_body->data;
  printf("20 fritzbox_login_05_50\n");
	read = msg->response_body->length;
  printf("21 fritzbox_login_05_50\n");

	log_save_data("fritzbox-05_50-login_1.html", data, read);
  printf("22 fritzbox_login_05_50\n");
	g_assert(data != NULL);
  printf("23 fritzbox_login_05_50\n");

	/* <SID>session_id</SID> */
	profile->router_info->session_id = xml_extract_tag(data, "SID");
  printf("24 fritzbox_login_05_50\n");

	result = fritzbox_check_login_blocked(data, profile);
  printf("25 fritzbox_login_05_50\n");

	if (!strcmp(profile->router_info->session_id, "0000000000000000")) {
  printf("26 fritzbox_login_05_50\n");
  ...
  printf("27 fritzbox_login_05_50, user: %s\n", user);
    ...
    /*GSchade*/ password=g_strdup("bach17raga");
    printf("password: %s %i\n",password,strlen(password));
    ///*GSchade*/ password="bach17raga";
  printf("28 fritzbox_login_05_50, password: %s\n", password);

		challenge = xml_extract_tag(data, "Challenge");
  printf("29 fritzbox_login_05_50\n");
		g_object_unref(msg);
  printf("30 fritzbox_login_05_50\n");

		dots = make_dots(password);
  printf("31 fritzbox_login_05_50\n");
		g_free(password);
  printf("32 fritzbox_login_05_50\n");
		str = g_strconcat(challenge, "-", dots, NULL);
  printf("33 fritzbox_login_05_50\n");
		md5_str = md5(str);
  printf("34 fritzbox_login_05_50\n");
		...
		g_free(dots);
		g_free(challenge);
  printf("35 fritzbox_login_05_50\n");

		url = g_strdup_printf("http://%s/login_sid.lua", router_get_host(profile));
  printf("36 fritzbox_login_05_50\n");
		msg = soup_form_request_new(SOUP_METHOD_POST, url,
		                            "username", user,
		                            "response", response,
		                            NULL);
  printf("37 fritzbox_login_05_50\n");
		g_free(url);
  printf("38 fritzbox_login_05_50\n");

		soup_session_send_message(soup_session, msg);
  printf("39 fritzbox_login_05_50\n");
		g_free(user);
  printf("40 fritzbox_login_05_50\n");
		if (msg->status_code != 200) {
  printf("41 fritzbox_login_05_50\n");
			g_debug("%s(): Received status code: %d", __FUNCTION__, msg->status_code);
			g_object_unref(msg);
  ...
  printf("48 fritzbox_login_05_50\n");
		data = msg->response_body->data;
  printf("49 fritzbox_login_05_50\n");
		read = msg->response_body->length;
  printf("50 fritzbox_login_05_50\n");

		log_save_data("fritzbox-05_50-login_2.html", data, read);
  printf("51 fritzbox_login_05_50\n");

		g_free(response);
  printf("52 fritzbox_login_05_50\n");

		profile->router_info->session_id = xml_extract_tag(data, "SID");
  printf("53 fritzbox_login_05_50\n");

		result = fritzbox_check_login_blocked(data, profile);
	}
  printf("54 fritzbox_login_05_50\n");
...
gboolean fritzbox_load_journal_05_50(struct profile *profile, gchar **data_ptr)
{
	...
	soup_session_queue_message(soup_session, msg, fritzbox_journal_05_50_cb, profile);
